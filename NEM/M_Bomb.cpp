...
extern int(__thiscall* getEnmObjFromId)(int* pId);
...
int __fastcall M_MarisaBombRelease(DWORD thiz)
{
	...
	attach_anm(pAnmFile, pAnmObj, 50);//bk
	HANM hAnm;
	insertAnmObj(&hAnm, pAnmObj);
	anm_ins_switch(pAnmObj);
...
{
	{
		...
		//{
		//	setAnmToNextStep(VALUED(thiz + 0x5C));
		//	VALUED(thiz + 0x5C) = 0;
		//	return -1;//end
		//}
		//return 0;//not end
	...
	if (time->time_last != time->time_now && time->time_now < 90)
	{
		...
		erase_bullet_round(r_now, (vector3f*)(thiz + 0x14), 0, 99999, 0);
		if (enmBase)
		{
					...
					if ((flag & 0x0000021) == 0)//available
					{
						...
						if (dist<r_now*r_now)
						{
							...
							if (!M_marisaBomb_enm_set.count(id))
							{
								...
								DWORD pAnmFile = VALUED(PPLAYER + 0xC);
								++VALUED(pAnmFile + 0x4D);
								attach_anm(pAnmFile, pAnmObj, 47);
								HANM hAnm;
								insertAnmObj(&hAnm, pAnmObj);
	...
	if (time->time_now == 120)
	{
		for (auto& i : M_marisaBomb_hanm_set)
		{
			...
			if (pAnm)
			{
				int n=summon_attack_circle(PPLAYER2, 10.0f, 0.5f, (vector3f*)(pAnm + 0x5F0), 90, 500);
				erase_bullet_round(100.0f, (vector3f*)(pAnm + 0x5F0), 0, 99999, 0);
				VALUED(PPLAYER + 0x204D8 + n * 0x9C+0x98) = 3;//fire eff
			}
			setAnmToNextStep(i.second);
		}
		setAnmToNextStep(VALUED(thiz + 0x64));
	}
