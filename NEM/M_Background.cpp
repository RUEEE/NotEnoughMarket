...
//	return 1;
//}
D3DMATERIAL9 ConvertMaterial(Material* material)
{
	D3DMATERIAL9 m;
	m.Ambient = { material->vAmbient.x,material->vAmbient.y,material->vAmbient.z,1 };
	m.Diffuse = { material->vDiffuse.x,material->vDiffuse.y,material->vDiffuse.z,1 };
	m.Emissive = {0,0,0,0};
	m.Power = static_cast<float>(material->nShininess);
	m.Specular = { material->vSpecular.x,material->vSpecular.y,material->vSpecular.z,1 };
	return m;
}

D3DXVECTOR3* g_pEyePos= (D3DXVECTOR3*)(0x004CCDF0 + 0x688 + 0x0);
D3DXVECTOR3* g_pUp_vec = (D3DXVECTOR3*)(0x004CCDF0 + 0x688 + 0x18);
D3DXVECTOR3* g_pFace = (D3DXVECTOR3*)(0x004CCDF0 + 0x688 + 0x24);
D3DXMATRIX* g_pMatPerspective = (D3DXMATRIX*)(0x004CCDF0 + 0x688 + 0xA0);
D3DXVECTOR3* g_pOffsNowPos = (D3DXVECTOR3*)(0x004CCDF0 + 0x688 + 0x3C);
D3DXMATRIX* g_pMatLookAt = (D3DXMATRIX*)(0x004CCDF0 + 0x688 + 0x60);
int* g_pRenderTarget_left = (int*)(0x004CD558);
int* g_pRenderTarget_top = (int*)(0x004CD55C);
int* g_pRenderTarget_width = (int*)(0x004CD560);
int* g_pRenderTarget_height = (int*)(0x004CD564);

D3DRECT GetClearRC()
{
	return { *g_pRenderTarget_left,*g_pRenderTarget_top,
		* g_pRenderTarget_left + *g_pRenderTarget_width,
		* g_pRenderTarget_top + *g_pRenderTarget_height };
}
...
int __fastcall M_RenderBG(DWORD thiz)
{
	...
	UINT bgRenderflag=VALUED(thiz + 0x3474);
	if (bgRenderflag & 0x8)
		return 1;



	D3DRECT clearRC=GetClearRC();
	...
	if (!is_read_file)
	{
		if (FAILED(meshLoader.Create(g_device, L"E:\\test\\ds22.obj")))
		{
	...
	if (is_suc)
	{
		...
		D3DXVECTOR3 at = { g_pFace->x + g_pEyePos->x,g_pFace->y + g_pEyePos->y,g_pFace->z + g_pEyePos->z };
		D3DXMatrixLookAtLH(&matView, g_pEyePos, &at, g_pUp_vec);
		g_device->SetTransform(D3DTS_VIEW, &matView);
		D3DXMATRIXA16 matProj;
		D3DXMatrixPerspectiveFovLH(&matProj, D3DX_PI / 4, 1.0f, 1.0f, 100.0f);
		g_device->SetTransform(D3DTS_PROJECTION, &matProj);

		if (meshLoader.GetNumMaterials() != -1)
		{
			for (DWORD i = 0; i < meshLoader.GetNumMaterials(); i++)
			{
				Material* pMaterial = meshLoader.GetMaterial(i);
				D3DMATERIAL9 material=ConvertMaterial(pMaterial);
				g_device->SetMaterial(&material);
				g_device->SetTexture(0, pMaterial->pTexture);
				meshLoader.GetMesh()->DrawSubset(i);
			}
		}else{
			Material* pMaterial = meshLoader.GetMaterial(1);
			D3DMATERIAL9 material = ConvertMaterial(pMaterial);
			g_device->SetMaterial(&material);
			g_device->SetTexture(0, pMaterial->pTexture);
			meshLoader.GetMesh()->DrawSubset(1);
		}
