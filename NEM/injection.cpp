...
HANDLE WINAPI MyCreateMutexA(LPSECURITY_ATTRIBUTES se, BOOL initialOwner, LPCSTR name)
{
    auto ret=RealCreateMutexA(se, initialOwner, name);
    auto err = GetLastError();
    if (err != ERROR_ALREADY_EXISTS)//successful
    {
        SetLastError(err);
        return ret;
    }
    auto boxRet=MessageBoxW(NULL, L"th18.exe still alive, use task manager to kill the exe,\n click YES to open process anyway", L"warning(touhou mod dll)", MB_YESNO);
    if (boxRet==IDNO)
    {
...
{
    hookIAT(GetModuleHandle(NULL), "Kernel32.dll", "CreateMutexA", MyCreateMutexA, (void**)&RealCreateMutexA);
}
