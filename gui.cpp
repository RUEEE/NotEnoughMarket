...
void SetGUI(IDirect3DDevice9* device)
{
	...
	ImGui::Begin("test");
	{
		if (VALUED(0x4CF2D0))
		{
			...
			ImGui::LabelText("-9918~-9915", "%.3f,%.3f,%.3f,%.3f", VALUEF(eb + 0x2C), VALUEF(eb + 0x30), VALUEF(eb + 0x34), VALUEF(eb + 0x38));
			ImGui::LabelText("-9922~-9919", "%.3f,%.3f,%.3f,%.3f", VALUEF(eb + 0x1C), VALUEF(eb + 0x20), VALUEF(eb + 0x24), VALUEF(eb + 0x28));
			static float scale=0.5f;
			ImGui::DragFloat("spd",&scale);
			ImGui::DragFloat4("-9918~-9915", (float*)(eb+ 0x2C),scale);
			ImGui::DragFloat4("-9922~-9919", (float*)(eb+ 0x1C),scale);
			static bool is_autoAcc = false;
			ImGui::Checkbox("autoAcc", &is_autoAcc);
			if (is_autoAcc)
			{
	...
	if (ImGui::Button("fuck the money"))
	{
	...
	if (keyDown_drop && (ImGui::GetMouseDragDelta().x!=0 || ImGui::IsMouseClicked(ImGuiMouseButton_Left)) && fabsf(smouse.x) < 192.0)
	{
		...
		DropItem(VALUED(0x004CF2EC), (Drop)drop, &mpos, i1[0], f1[0], f1[1], i1[1], i1[2], i1[3]);
		//drop++;
	}
